...
class Maze(object):
    ...
    def sensor_model(self, coordinates, sensor_limit, orientation = 0):
        ...
        pos_y = y
        d6 = 0
        dx = np.cos(orientation-np.pi/4*3) * 1 - np.sin(orientation-np.pi/4*3) * 0
        dy = np.sin(orientation-np.pi/4*3) * 1 + np.cos(orientation-np.pi/4*3) * 0
